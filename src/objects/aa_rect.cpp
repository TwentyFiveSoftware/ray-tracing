...
AxisAlignedRectangle::AxisAlignedRectangle(float x0, float x1, float y0, float y1, float z,
                                           std::shared_ptr<Material> material) :
        x0(x0), x1(x1), y0(y0), y1(y1), z(z), materialPtr(std::move(material)) {}

bool AxisAlignedRectangle::hit(const Ray &ray, float tMin, float tMax, HitRecord &record) const {
...
bool AxisAlignedRectangle::boundingBox(AABB &outputBox) const {
    outputBox = AABB(
